<?php if ( have_posts() ) : ?>
	...
	<?php
	$next_posts_link     = get_next_posts_link( esc_html__( 'Next Entries »', 'crb' ) );
	$previous_posts_link = get_previous_posts_link( esc_html__( '« Previous Entries', 'crb' ) );
	if ( $next_posts_link || $previous_posts_link ) :
	?>
		<div class="paging">
			<?php printf( '%s %s', $previous_posts_link, $next_posts_link ); ?>
		</div><!-- /.paging -->
	<?php endif; ?>
<?php else : ?>
	<div class="articles">
